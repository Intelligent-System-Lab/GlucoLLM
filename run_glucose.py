...
parser.add_argument('--llm_model', type=str, default='GPT2', help='LLM model') # LLAMA, GPT2, BERT, DEEPSEEK
parser.add_argument('--llm_dim', type=int, default='768', help='LLM model dimension')# LLama7b:4096; GPT2-small:768; BERT-base:768; Deepseek-1.3b:2048
...
# === Add: Load age and gender info from dataset/age-gender.csv ===
age_gender_path = './dataset/age-gender.csv'
if os.path.exists(age_gender_path):
    import pandas as pd
    df = pd.read_csv(age_gender_path)
    if 'ID' in df.columns:
        row = df[df['ID'].astype(str) == str(args.model_id)]
        if not row.empty:
            args.age = row.iloc[0]['age']
            args.gender = row.iloc[0]['gender']
        else:
            args.age = None
            args.gender = None
    else:
        print("Error: 'ID' column not found in age-gender.csv. Available columns:", df.columns.tolist())
        args.age = None
        args.gender = None
else:
    args.age = None
...
for ii in range(args.itr):
    ...
    # Load content before checking separate_test
    ...
    if args.separate_test == 'yes':
        ...
        # Set prompt domain (should be consistent with dataset)
        ...
        for epoch in range(args.train_epochs):
            ...
            accelerator.print(
                #"Epoch: {0} | Train Loss: {1:.7f} Vali Loss: {2:.7f} Test Loss: {3:.7f} MAE Loss: {4:.7f} RMSE Loss: {5:.7f}".format(
